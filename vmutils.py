...
LOG = logging.getLogger("endlesscode")
...
class VMUtils(object):
    ...
    def _lookup_vm_check(self, vm_name):
        ...
        if not vm:
            raise Exception("No found vm %s" % vm_name)
        return vm
    ...
    def check_admin_permissions(self):
        if not self._conn.Msvm_VirtualSystemManagementService():
            msg = "The Windows account running nova-compute on this Hyper-V" +\
    ...
    def create_vm(self, vm_name, memory_mb, vcpus_num, limit_cpu_features,
        ...
        vm = self._create_vm_obj(vs_man_svc, vm_name, notes, dynamic_memory_ratio)
    ...
    def attach_ide_drive(self, vm_name, path, ctrller_addr, drive_addr,
        ...
        ctrller_path = self._get_vm_ide_controller(vm, ctrllere_addr)
    ...
    def set_disk_host_resource(self, vm_name, controller_path, address,
        ...
        for disk_resource in disk_resources + volume_resources:
            if (disk_resource.Parent == controller_path and
                ...
                break
        if not disk_found:
            LOG.warn('Disk not found on controller "%(controller_path)s" '
    ...
    def create_nic(self, vm_name, nic_name, mac_address=None):
        ...
        #new_nic_data.Address = "10.0.0.11"
    ...
    def _wait_for_job(self, job_path):
        ...
        if job.JobState != constants.WMI_JOB_STATE_COMPLETED:
            ...
            if job.path().Class == "Msvm_ConcreteJob":
                ...
                raise HyperVException("WMI job failed with status "
    ...
    def get_processor_load_percentage(self, vm_name=None):
        """Return vm's cpu usage history as tuple, if vm_name not give, list all"""
        vm_id = None
        rst = dict()
        if(vm_name):
            vm_id = self.get_vm_id(vm_name)
        processors = self._conn.MSVM_Processor()
        for proc in processors:
            if vm_id and vm_id == proc.SystemName:
                return {vm_id: proc.LoadPercentage}
            else:
                rst[proc.SystemName] = proc.LoadPercentage
